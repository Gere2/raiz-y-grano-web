  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute(
        'content',
        'Descubre nuestra carta completa de café de especialidad, bollería artesanal, zumos y más en Raíz y Grano.'
      );
    }
    window.scrollTo(0, 0);
  }, []);
      ...
      <section className="pt-32 pb-16 px-4" style={{ background: "linear-gradient(135deg, #ede3cc 0%, #e0d4bb 100%)" }}>
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="max-w-2xl mx-auto text-[#7d6a50] mb-10 font-light italic">
            Artesanía, naturalidad y un café inolvidable.<br />
      ...
      <section className="py-12 px-4">
        <div className="max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {category.items.map((item, index) => (
                  <div 
                    ...
                    style={{
                      boxShadow: '0 2px 10px 0 rgba(190,170,140,0.05)'
                    }}
                  >
                    <div className="flex justify-between items-start">
                      <h3 className="font-medium text-[#5a442c] tracking-tight font-cormorant">{item.name}</h3>
                      <span className="font-semibold text-[#a18968] ml-4 bg-[#e9e0d1] px-2 py-0.5 rounded-full text-sm font-opensans">{item.price}</span>
                    </div>
                    <p className="text-sm opacity-80 mt-2 text-[#96836a] font-opensans">{item.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-16 text-center">
            <p className="italic text-[#8f795f] opacity-70 mb-4">¿Alergias o preferencias? Consúltanos sin compromiso.</p>
